...
先介绍背景。我的电脑上 `camsvc`（Capability Access Manager Service，能力访问管理器）的 `SQLite` 数据库不知什么原因，WAL 日志文件统共涨到了 **40 多 GB**，把磁盘吃干抹净。我想着删掉几个数据库文件清清空间，就干了最直接的一件事——
...
然后把这个目录下所有文件删了个精光。毕竟 `Windows` 不拿所有权根本不让删。
